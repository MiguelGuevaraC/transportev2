...
class PdfController extends Controller
{
    ...
    /**
     * @OA\Get(
     *     path="/transportev2/public/api/saveguia/{id}",
    ...
    /**
     * @OA\Get(
     *     path="/transportev2/public/api/manifiesto/{id}",
...
// Combina ambas fuentes y elimina duplicados
    ...
    /**
     * @OA\Get(
     *     path="/transportev2/public/api/reportCaja",
...
// Verificamos si la URL es válida (no es null o vacía)
    ...
    /**
     * @OA\Get(
     *     path="/transportev2/public/api/reportCajaExcel",
...
// REPORTE DE RECEPCIONES EN EXCEL
    public function reporteRecepcionesExcel(Request $request)
    {
        ...
        if ($branch_office_id) {
            ...
            // En caso de no recibir `branch_office_id`, asigna uno por defecto (ejemplo: 3)
            // $branch_office_id = 3;
            // $branchOffice = BranchOffice::find($branch_office_id);
        }
        ...
        $receptions = Reception::with([
            ...
            'seller', 'pointDestination', 'pointSender',
        ...
        if (! empty($origenOrDestino)) {
            ...
            $receptions->where(function ($query) use ($origenOrDestinoUpper) {
                $query->whereHas('origin', function ($q) use ($origenOrDestinoUpper) {
                    $q->where(DB::raw('UPPER(name)'), 'LIKE', '%' . $origenOrDestinoUpper . '%');
                })->orWhereHas('destination', function ($q) use ($origenOrDestinoUpper) {
                    $q->where(DB::raw('UPPER(name)'), 'LIKE', '%' . $origenOrDestinoUpper . '%');
                });
            });
        }
        ...
        if (! empty($isCargos)) {
            if ($isCargos == "true") {
                ...
                $receptions->whereDoesntHave('cargos');
            }
        }
        ...
        foreach ($receptions as $reception) {
            ...
            $carga = DB::table('detail_receptions')
                ->where('reception_id', $recept->id)
                ->pluck('description')
                ->implode(', ') ?? '-';
            ...
            // Acumular totales
            $flete += (float) ($recept->paymentAmount ?? 0);
            $deuda += (float) ($recept->debtAmount ?? 0);
            $peso += (float) ($recept->netWeight ?? 0);
        }
